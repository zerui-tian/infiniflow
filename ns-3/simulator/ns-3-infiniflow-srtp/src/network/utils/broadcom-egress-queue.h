...
namespace ns3 {
	...
	public:
		...
		static const unsigned fCnt = F_CNT; //max number of queues, 128 for NICs
		static const unsigned qCnt = Q_CNT; //max number of queues, 8 for switches
		BEgressQueue();
		virtual ~BEgressQueue();
		bool Enqueue(Ptr<Packet> p, uint32_t qIndex);
		Ptr<Packet> DequeueSRTP(uint32_t credit, uint32_t inflight[], uint32_t thresholds[]);
	...
	private:
		...
		Ptr<Packet> DoDequeueSRTP(uint32_t credit, uint32_t inflight[], uint32_t thresholds[]);
		//for compatibility
