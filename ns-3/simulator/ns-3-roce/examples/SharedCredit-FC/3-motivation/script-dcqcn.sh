...
config_file="$config_dir/config_dcqcn.txt"
...
for buffer_mul in "${BUFFER_SIZE_LIST[@]}"; do
    ...
    for vc_cnt in "${VC_CNT_LIST[@]}"; do
        ...
        ############################################
        FCT_FILE="$OUTPUT/3-motivation/buf${buffer_mul}_vc${vc_cnt}_dcqcn.txt"
        STDOUT_FILE="$output_dir/buf${buffer_mul}_vc${vc_cnt}_dcqcn.out"
        ...
        ############################################
        {
            cat "$config_dir/config_template_dcqcn.txt"
        ...
        # Run experiment
        ############################################
        ./waf --run "3-motivation --conf=$config_file --windowCheck=0" \
            > "$STDOUT_FILE" || {
