...
config_file="$config_dir/config_common.txt"
...
for buffer_mul in "${BUFFER_SIZE_LIST[@]}"; do
    ...
    for vc_cnt in "${VC_CNT_LIST[@]}"; do
        ...
        # Output files
        ############################################
        FCT_FILE="$OUTPUT/3-motivation/buf${buffer_mul}_vc${vc_cnt}_pfc.txt"
        STDOUT_FILE="$output_dir/buf${buffer_mul}_vc${vc_cnt}_pfc.out"
        ...
        ############################################
        {
            cat "$config_dir/config_template_common.txt"
        ...
        # gdb --args build/examples/SharedCredit-FC/ns3.39-3-motivation-optimized --conf=$config_file --windowCheck=1
