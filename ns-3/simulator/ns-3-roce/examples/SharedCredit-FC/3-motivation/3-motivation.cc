...
double rate_decrease_interval = 4, m_nack_interval = 1;
...
int main(int argc, char *argv[])
{
	...
	while (!conf.eof())
	{
		...
		else if (key.compare("NACKGenerationInterval") == 0)
		{
			double v;
			conf >> v;
			m_nack_interval = v;
			std::cout << "NACKGenerationInterval\t\t" << m_nack_interval << "\n";
		}
	...
	// config switch
	uint64_t per_port_buffer = buffer_size / SwitchMmu::pCnt;
		...
		if (n.Get(i)->GetNodeType()) { // is switch
			...
			for (uint32_t j = 1; j < sw->GetNDevices(); j++) {
				...
				for (uint32_t qu = 0; qu < SwitchMmu::qCnt ; qu++) {
					...
					sw->m_mmu->ConfigEcn(j, (double)bs * 0.15 / 1000, (double) bs * 0.3 / 1000, rate2pmax[rate]);
					// std::cout << "kmin " << (double)bs * 0.15 / 1000<<std::endl;
					sw->m_mmu->ConfigThreshold(j, qu, bs / 3, bs * 2 / 3);
					// std::cout << bs / 3 <<  "  " << bs * 2/ 3 <<std::endl;
				}
			}
		}
	}
	...
	for (uint32_t i = 0; i < node_num; i++) {
		if (n.Get(i)->GetNodeType() == 0) { // is server
			...
			rdmaHw->SetAttribute("NACKGenerationInterval", DoubleValue(m_nack_interval));
