...
int main(int argc, char *argv[])
{
	...
	buffer_size = 40000000; // total buffer size for each switch
	uint32_t per_port_buffer = buffer_size / SwitchMmu::pCnt; 
	std::cout << "switch buffer size " << buffer_size << std::endl;
	for (uint32_t i = 0; i < node_num; i++) {
		if (n.Get(i)->GetNodeType()) { // is switch
			...
			for (uint32_t j = 1; j < sw->GetNDevices(); j++) {
				...
				for (uint32_t qu = 0; qu < SwitchMmu::qCnt ; qu++) {
					...
					// std::cout << "xon " << leaf_spine_bdp << " xoff " << bs - leaf_spine_bdp << std::endl;
				}
			}
		}
	}
