...
output_dir="$OUTPUT/16-large-scale/dcqcn"
config_file="$config_dir/config-dcqcn.txt"
...
for model in "${modelNames[@]}"; do
    ...
    for load in "${loadNums[@]}"; do
        ...
        # 生成配置文件内容
        {
            cat "$config_dir/config-scale-dcqcn.txt"
        ...
        # 运行实验
        ./waf --run "16-large-scale --conf=$config_file --windowCheck=0" > "$output_dir/output/${model}_load${load}.out"|| {
