...
int main(int argc, char *argv[])
{
	...
	for (uint32_t i = 0; i < node_num; i++) {
		if (n.Get(i)->GetNodeType()) { // is switch
			...
			sw->m_mmu->ConfigBufferSize(buffer_size);
			...
			for (uint32_t j = 1; j < sw->GetNDevices(); j++) {
				...
				uint64_t bdp = rate * delay * 2 / 8 / 1000000000;
				
				for(uint32_t qu = 0;qu < SwitchMmu::qCnt; qu++) {
					uint32_t bs = sw->m_mmu->GetBufferSize(j, qu);
					sw->m_mmu->ConfigThreshold(j, qu, bdp, bs - bdp);
					std::cout << "xon " << bdp << " xoff " << bs - bdp << std::endl;
				}
