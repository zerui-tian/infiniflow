...
std::string flowthroughput_output_file;
...
uint32_t flowthroughput_dump_interval;
uint32_t flowthroughput_mon_interval;
uint32_t flowthroughput_mon_start;
uint32_t flowthroughput_mon_end;
...
uint32_t buffer_size = 125; //BDP = 64250000 Bytes for 25Gbps 5us
uint32_t maxThreshold = 100000; 

uint32_t p_queue_count = 8;
float qmin = 3;
float qmax = 6;
				...
				for(uint32_t j = 0; j < host->m_nic.size(); j++){
					...
					if(nic != NULL){
						...
						for(uint32_t k = 0; k < qpGrp->m_qps.size(); k++){
							qpGrp->m_qps[k]->getFlowTxBytes(); //使用的numTxBytes
						}
					}
				}
			}
			...
			if (n->Get(i)->GetNodeType() == 0){ //是主机
				...
				for(uint32_t j = 0; j < host->m_nic.size(); j++){
					...
					if(nic != NULL){
						Ptr<RdmaQueuePairGroup> qpGrp = nic->m_rdmaEQ->m_qpGrp;
						for(uint32_t k = 0; k < qpGrp->m_qps.size(); k++){
							Ptr<RdmaQueuePair> qp = qpGrp->m_qps[k];
	...
	if(Simulator::Now().GetTimeStep() == throughput_mon_start){ //统计开始时清零
		...
		for(uint32_t i = 0; i < nd->GetN(); i++){
			...
			new_item.throughput_Gbps = 
				(double)(output_device->getNumTxBytes() * 8) / throughput_mon_interval; //使用的totalBytesSent
...
std::vector<FlowThroughputEntry> flowthroughput_waveform;

void monitor_flowlink(FILE* flowthroughput_output, NetDeviceContainer* nd,std::vector<uint32_t> target_fids) {
    if (Simulator::Now().GetTimeStep() == flowthroughput_mon_start) { //统计开始时清零
        for (uint32_t i = 0; i < nd->GetN(); i++) {
            Ptr<QbbNetDevice> dev = DynamicCast<QbbNetDevice>(nd->Get(i));
            for (uint32_t flow_id : target_fids) {
				dev->m_txBytesPerFlow[flow_id] = 0;
			}
        }
    } else {
        for (uint32_t i = 0; i < nd->GetN(); i++) {
            ...
            // ✅ 遍历目标流数组
            for (uint32_t flow_id : target_fids) {
                uint64_t bytes = output_device->getNumTxBytesByFlow(flow_id); 
				//重置记录发送的字节数
				output_device->m_txBytesPerFlow[flow_id] = 0;
                FlowThroughputEntry new_item;
                new_item.time_ns = Simulator::Now().GetNanoSeconds();
                new_item.sid = output_device->GetNode()->GetId();
                new_item.rid = input_device->GetNode()->GetId();
                new_item.flow_id = flow_id; // ✅ 记录flow id
                new_item.throughput_Gbps = (double)(bytes * 8) / flowthroughput_mon_interval;
                flowthroughput_waveform.push_back(new_item);
            }
        }

        if (Simulator::Now().GetTimeStep() % flowthroughput_dump_interval == 0) {
            for (auto& entry : flowthroughput_waveform) {
                // ✅ 输出中新增flow_id列
                fprintf(flowthroughput_output, "%u %u %u %.2f %u\n",
                        entry.sid, entry.rid, entry.flow_id,
                        entry.throughput_Gbps, entry.time_ns);
                fflush(flowthroughput_output);
            }
            flowthroughput_waveform.clear();
        }
    }

    if (Simulator::Now().GetTimeStep() < flowthroughput_mon_end) {
        Simulator::Schedule(NanoSeconds(flowthroughput_mon_interval),
                            monitor_flowlink, flowthroughput_output, nd, target_fids);
    }
}
// typedef struct {
// 	int64_t time_ns;
...
// 				uint32_t size = DynamicCast<QbbNetDevice>(sw->GetDevice(j))->m_queue->GetNBytes(DATA_Q_IDX);
// 				qlen_dist[i][j].add(size);
...
int main(int argc, char *argv[])
{
	...
	while (!conf.eof())
	{
		...
		else if (key.compare("ERROR_RATE_PER_LINK") == 0)
		{
			...
			std::cout << "ENABLE_TRACE\t\t\t\t" << enable_trace << '\n';
				} else if (key.compare("QMIN") == 0) {
			conf >> qmin;
			std::cout << "QMIN\t\t\t\t" << qmin << '\n';
		} else if (key.compare("QMAX") == 0) {
			conf >> qmax;
			std::cout << "QMAX\t\t\t\t" << qmax << '\n';
		...
		}  else if (key.compare("FLOWTHROUGHPUT_DUMP_INTERVAL") == 0) {
			conf >> flowthroughput_dump_interval;
			std::cout << "FLOWTHROUGHPUT_DUMP_INTERVAL\t\t" << flowthroughput_dump_interval << "\n";
		} else if (key.compare("FLOWTHROUGHPUT_MON_INTERVAL") == 0) {
			conf >> flowthroughput_mon_interval;
			std::cout << "FLOWTHROUGHPUT_MON_INTERVAL\t\t" << flowthroughput_mon_interval << "\n";
		} else if (key.compare("FLOWTHROUGHPUT_MON_START") == 0) {
			conf >> flowthroughput_mon_start;
			std::cout << "FLOWTHROUGHPUT_MON_START\t\t\t\t" << flowthroughput_mon_start << '\n';
		} else if (key.compare("FLOWTHROUGHPUT_MON_END") == 0) {
			conf >> flowthroughput_mon_end;
			std::cout << "FLOWTHROUGHPUT_MON_END\t\t\t\t" << flowthroughput_mon_end << '\n';
		} else if (key.compare("FLOWTHROUGHPUT_MON_FILE") == 0) {
			conf >> flowthroughput_output_file;
			std::cout << "FLOWTHROUGHPUT_MON_FILE\t\t\t\t" << flowthroughput_output_file << '\n';
	...
	std::cout << "Finished creating channels." << std::endl;
	...
	uint32_t leaf_spine_bdp = 100 * 3000 * 2 / 8; // 100Gbps, 500ns delay, 1 BDP(bytes)
	uint32_t per_port_buffer =  2 * leaf_spine_bdp; // per port buffer size **这个修改为自己设置的buffer
	buffer_size = per_port_buffer * SwitchMmu::pCnt;
	for (uint32_t i = 0; i < node_num; i++) {
		if (n.Get(i)->GetNodeType()) { // is switch
			Ptr<SwitchNode> sw = DynamicCast<SwitchNode>(n.Get(i));
			// uint64_t bdp_in_bytes;
			for (uint32_t j = 1; j < sw->GetNDevices(); j++) {
				Ptr<QbbNetDevice> dev = DynamicCast<QbbNetDevice>(sw->GetDevice(j));
				for (uint32_t qu = 0; qu < SwitchMmu::qCnt ; qu++) {
					...
					sw->m_mmu->maxThreshold = per_port_buffer;
					sw->m_mmu->SetInflightThreshold(j, qu, per_port_buffer);
				}
			}
			sw->m_mmu->SetQlenMax(qmax*packet_size);
			sw->m_mmu->SetQlenMin(qmin*packet_size);
			sw->m_mmu->SetAiStep(packet_size / 2);
			sw->m_mmu->SetBufferPool(buffer_size); //将buffer均分给每个port的每个queue
	...
	for (uint32_t i = 0; i < node_num; i++) {
		if (n.Get(i)->GetNodeType() == 0) { // is server
			...
			for(int j = 0;j < rdmaHw->m_nic.size(); j++) {
				Ptr<QbbNetDevice> dev = rdmaHw->m_nic[j].dev;
				if (dev == NULL)
					continue;		
				dev->perpt_fccl = per_port_buffer;
				dev->m_rdmaEQ->m_mtu = packet_payload_size;
				dev->SetQlenMax(qmax*packet_size);
				dev->SetQlenMin(qmin*packet_size);
				dev->SetAiStep(packet_size / 2);
				
				// uint64_t rate = dev->GetDataRate().GetBitRate();
				// uint64_t delay = DynamicCast<QbbChannel>(dev->GetChannel())->GetDelay().GetNanoSeconds();
				// set inflight threshold
				// uint64_t bdp_in_bytes = rate * delay / 1000000000 / 8 * 2;
				dev->maxThreshold = per_port_buffer;
				for(int qu = 0; qu < QbbNetDevice::qCnt; qu++){
					dev->perpg_inflight_threshold[qu] = per_port_buffer;
				}
			}
		}
	}
	...
	std::cout << "Total Flows: " << flow_num << std::endl;
	topof.close();
	tracef.close();
	...
	FILE *flowthroughput_file = fopen(flowthroughput_output_file.c_str(), "w");
	std::vector<uint32_t> target_fids = {10000, 10001, 10002, 10003};

	Simulator::Schedule(NanoSeconds(flowthroughput_mon_start), monitor_flowlink, flowthroughput_file, &nd, target_fids);
