...
output_dir="$OUTPUT/16-large-scale/infiniflow"
...
for model in "${modelNames[@]}"; do
    ...
    for load in "${loadNums[@]}"; do
        ...
        # 运行实验
        ./waf --run "16-large-scale --conf=$config_file" > "$output_dir/output/${model}_load${load}.out"|| {
            echo "Experiment ${model}_load${load} failed!"
            continue
        }
        # gdb --args build/examples/infiniflow/ns3.39-16-large-scale-optimized --conf=$config_file
