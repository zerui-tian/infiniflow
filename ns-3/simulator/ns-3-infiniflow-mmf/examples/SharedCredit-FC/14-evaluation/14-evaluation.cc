...
int main(int argc, char *argv[])
{
	...
	buffer_size = SwitchMmu::pCnt * (2 * 100 * 3000 * 2 / 8);
	uint32_t per_port_buffer = buffer_size / SwitchMmu::pCnt;
	for (uint32_t i = 0; i < node_num; i++) {
		if (n.Get(i)->GetNodeType()) { // is switch
			Ptr<SwitchNode> sw = DynamicCast<SwitchNode>(n.Get(i));
			// sw->m_mmu->qCnt = p_queue_count;
			for (uint32_t j = 1; j < sw->GetNDevices(); j++) {
				// std::cout << "qCnt : " << SwitchMmu::qCnt << std::endl;
				for (uint32_t qu = 0; qu < SwitchMmu::qCnt ; qu++) {
					...
					// dev->qCnt = p_queue_count;
					// dev->m_rdmaEQ->qCnt = p_queue_count;
					// set ecn
