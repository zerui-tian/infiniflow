...
int main(int argc, char *argv[])
{
	...
	// uint32_t leaf_spine_bdp = 100 * 500 * 2 / 8; // 100Gbps, 500ns delay, 1 BDP(bytes)
	uint32_t per_port_buffer = 64 * packet_size; // per port buffer size
	std::cout << " per port buffer " << per_port_buffer << std::endl;
	...
	for (uint32_t i = 0; i < node_num; i++) {
		if (n.Get(i)->GetNodeType() == 0) { // is server
			...
			for(int j = 0;j < rdmaHw->m_nic.size(); j++) {
				...
				if (dev == NULL)
					continue;
				dev->perpt_fccl = buffer_size / SwitchMmu::pCnt;
	...
	// std::vector<uint32_t> target_fids = {10000,10001};
