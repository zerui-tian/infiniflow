...
int main(int argc, char *argv[])
{
	...
	buffer_size = 40 * 1024 * 1024;//40MB
	uint32_t per_port_buffer = buffer_size / SwitchMmu::pCnt;
	// uint32_t leaf_spine_bdp = 100 * 1000 * 2 / 8; // 100Gbps, 500ns delay, 1 BDP(bytes)
	for (uint32_t i = 0; i < node_num; i++) {
		if (n.Get(i)->GetNodeType()) { // is switch
			Ptr<SwitchNode> sw = DynamicCast<SwitchNode>(n.Get(i));
			sw->m_mmu->ConfigBufferSize(buffer_size); //将buffer均分给每个port的每个queue
			...
			for (uint32_t j = 1; j < sw->GetNDevices(); j++) {
				...
				for (uint32_t qu = 0; qu < SwitchMmu::qCnt ; qu++) {
					...
					uint32_t bs = sw->m_mmu->GetBufferSize(j, qu);
					sw->m_mmu->ConfigThreshold(j, qu, bs / 3 , bs * 2 / 3);
					// std::cout << "xon " << bs / 3 << " xoff " << bs * 2 / 3 << std::endl;
				}
			}
		}
	}
