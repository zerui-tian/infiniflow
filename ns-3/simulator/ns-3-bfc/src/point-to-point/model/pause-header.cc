...
PauseHeader::PauseHeader (uint32_t time, uint32_t qlen, uint16_t qindex)
...
void PauseHeader::SetQIndex (uint16_t qindex)
{
  m_qindex = qindex;
}

uint16_t PauseHeader::GetQIndex () const
{
  return m_qindex;
}
