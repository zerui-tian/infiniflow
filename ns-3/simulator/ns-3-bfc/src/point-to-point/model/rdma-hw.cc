...
void RdmaHw::AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish, Time stopTime) {
	...
	// std::cout << "add queuepair flowid " << dport << std::endl;
	qp->SetSize(size);
	qp->SetWin(win);
	...
	// std::cout << "dip " << dip.Get() << "sport " << sport << "pg " << pg << std::endl;
	...
	// 动态分配
	int i = 1;
	// std::cout << "qCnt" << QbbNetDevice::qCnt << std::endl;
	for(;i < QbbNetDevice::qCnt; i++){
		if(m_nic[nic_idx].dev->vl_states[i] == 0){
			m_nic[nic_idx].dev->vl_states[i] = 1;
			m_nic[nic_idx].dev->flow_qIndex_map[dport] = i;
			// std::cout << "[INSERT] map address = " << &m_nic[nic_idx].dev->flow_qIndex_map << std::endl;
			// std::cout << "qbbid " << m_nic[nic_idx].dev->GetIfIndex() <<" Assign virtual lane " << i << " for qp" << dport << std::endl;
			qp->m_vl = i;
			break;
		}
	}
	if(i == QbbNetDevice::qCnt){
		int range = QbbNetDevice::qCnt - 1;
		uint32_t vl = 1 + (rand() % range);
		m_nic[nic_idx].dev->vl_states[vl] += 1;
		m_nic[nic_idx].dev->flow_qIndex_map[dport] = vl;
		qp->m_vl = vl;
		std::cout << "No available virtual lane for qp" << dport << std::endl;
	}
	// Notify Nic
...
void RdmaHw::DeleteQueuePair(Ptr<RdmaQueuePair> qp) {
	...
	uint32_t nic_idx = GetNicIdxOfQp(qp);
	m_nic[nic_idx].dev->vl_states[qp->m_vl] -= 1;
	// std::cout << "delete queuepair flowid " << qp->dport << std::endl;
	m_qpMap.erase(key);
}
...
int RdmaHw::ReceiveUdp(Ptr<Packet> p, CustomHeader &ch) {
	...
	// std::cout << "hw receive " << ch.udp.vl << std::endl;
	...
	if (x == 1 || x == 2) { //generate ACK or NACK
		...
		// std::cout << "ReceiverCheckSeq returns " << x << std::endl;
	}
	return 0;
}
...
int RdmaHw::ReceiveAck(Ptr<Packet> p, CustomHeader &ch) {
	...
	if (qp == NULL) {
		std::cout << "ERROR: " << "node:" << m_node->GetId() << " port " << port << " qIndex " << qIndex  << " flowid " << ch.ack.sport << " seq " << ch.ack.seq << " "<< (ch.l3Prot == 0xFC ? "ACK" : "NACK") << " NIC cannot find the flow\n";
		return 0;
	}
	// std::cout << "receive ack " << ch.l3Prot <<std::endl;
	uint32_t nic_idx = GetNicIdxOfQp(qp);
	Ptr<QbbNetDevice> dev = m_nic[nic_idx].dev;
	if (m_ack_interval == 0)
	...
	{	
		// if(ch.ack.sport == 15692)
		// 	std::cout << "recover flowid " << ch.ack.sport << " seq " << ch.ack.seq << std::endl;
		RecoverQueue(qp);
	}
...
int RdmaHw::ReceiverCheckSeq(uint32_t seq, Ptr<RdmaRxQueuePair> q, uint32_t size) {
	...
	if (seq == expected) {
		...
		// if(q->sport == 15692) std::cout << "update ReceiverNextExpectedSeq " << q->ReceiverNextExpectedSeq << std::endl;
		...
		if (Simulator::Now() >= q->m_nackTimer || q->m_lastNACK != expected) {
			...
			// std::cout << "send NACK flowid " << q->sport << " seq " << q->ReceiverNextExpectedSeq << std::endl;
			return 2;
		} else
			return 4;
	} else {
...
Ptr<Packet> RdmaHw::GetNxtPacket(Ptr<RdmaQueuePair> qp) {
	...
	qp->numTxBytes = qp->numTxBytes + payload_size + CustomHeader::GetStaticWholeHeaderSize();
	// return
	return p;
}
