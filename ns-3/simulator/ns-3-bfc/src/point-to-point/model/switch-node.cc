...
void SwitchNode::CheckAndSendPfc(uint32_t inDev, uint32_t qIndex) {
	...
	if (m_mmu->CheckShouldPause(inDev, qIndex)) {
		...
		// std::cout << "sending PFC " << " ingress " << m_mmu->ingress_bytes[inDev][qIndex] 
		// << " xoff " << m_mmu->xoff[inDev][qIndex] 
		// << " qIndex " << qIndex <<std::endl;
		m_mmu->SetPause(inDev, qIndex);
		// std::cout << "drop switch " << GetId() << " port " << inDev << " qIndex " << qIndex << std::endl;
		// std::cout << " buffer_size " << m_mmu->GetBufferSize(inDev , qIndex) << " ingress_bytes " << m_mmu->ingress_bytes[inDev][qIndex] << std::endl;
	}
}
...
void SwitchNode::SendToDev(Ptr<Packet> p, CustomHeader &ch) {
	...
	if (idx >= 0) {
		...
		else
        {
            ...
            if(it == m_mmu->bytes_in_switch.end() || it->second == 0){
                ...
                if(i == qCnt){
					...
					m_mmu->active_vl[idx] += 1;
					if(m_mmu->active_vl[idx] > 1)
						m_mmu->threshold[idx] = max_bdp / m_mmu->active_vl[idx];
					else 
						m_mmu->threshold[idx] = max_bdp;
				}
			}
			else {
		...
		if (downstream_vl != 0) { //not highest priority
			...
			{			// Admission control
				...
				// std::cout << "sw " << downstream_vl << std::endl;
				if(m_mmu->egress_bytes[idx][downstream_vl] > m_mmu->threshold[idx])
				{
					// std::cout << "Time " << Simulator::Now().GetNanoSeconds() 
					// 	<< " egressbytes " << m_mmu->egress_bytes[idx][downstream_vl]
					// 	<< " node " << GetId() 
					// 	<< " port " << idx
					// 	<< " qIndex " << downstream_vl
					// 	<< std::endl;
					Ptr<QbbNetDevice> device = DynamicCast<QbbNetDevice>(m_devices[inDev]);
					// 打标记为true
					CounterIncrTag ct;
					ct.SetTag(1);
					// std::cout << "add tag " << ct.GetTag() << std::endl;
					p->AddPacketTag(ct);
					// std::cout << "add tag " << ct.GetTag() << std::endl;
					m_mmu->pauseCounter[inDev][upstream_vl] += 1;
					// std::cout << "pause counter " << m_mmu->pauseCounter[inDev][upstream_vl] 
					// << " node " << GetId() 
					// << " port " << inDev 
					// << " qIndex " << upstream_vl 
					// << std::endl;
					if(m_mmu->pauseCounter[inDev][upstream_vl] == 1){
						device->SendPfc(upstream_vl, 0);
						// std::cout << "send pause " << upstream_vl << std::endl;
					}
				}
				else {
					CounterIncrTag ct;
				...
				// std::cout << "drop switch " << GetId() << " port " << inDev << " qIndex " << qIndex << " packet_size "<< p->GetSize() << " protocal " << ch.l3Prot << std::endl;
				// std::cout << " buffer_size " << m_mmu->GetBufferSize(inDev , qIndex) << " ingress_bytes " << m_mmu->ingress_bytes[inDev][qIndex] << std::endl;
				return; // Drop
			}
			// CheckAndSendPfc(inDev, upstream_vl);
			// bfc发送pause帧
		}
		...
		// std::cout << "switch " << GetId() << " port " << idx << " qIndex " << qIndex << 
		// " bs " << m_mmu->ingress_bytes[idx][qIndex] << " time " << Simulator::Now().GetNanoSeconds() << std::endl;
	} 
	else
...
void SwitchNode::SwitchNotifyDequeue(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p) {
    ...
    if (ch.l3Prot == 0xFF || ch.l3Prot == 0xFE || ch.l3Prot == 0xFC){ // QCN or PFC or NACK, go ack
		...
		if(m_mmu->bytes_in_switch[flowid] == 0) {
			...
			if(m_mmu->vl_states[ifIndex][downstream_vl] == 0) {
				m_mmu->active_vl[ifIndex] -= 1;
				if(m_mmu->active_vl[ifIndex] > 1)
					m_mmu->threshold[ifIndex] = max_bdp / m_mmu->active_vl[ifIndex];
				else 
					m_mmu->threshold[ifIndex] = max_bdp;
			}
		}
	...
	if (qIndex != 0) {
		...
		// std::cout << "switch " << GetId() << " port " << inDev << " qIndex " << qIndex << 
		// " bs " << m_mmu->ingress_bytes[inDev][qIndex] << " time " << Simulator::Now().GetNanoSeconds() << std::endl;
		if (m_ecnEnabled) {
		...
		if (tag == 1) {
			...
			// << std::endl;;
			// std::cout << "Time " << Simulator::Now().GetNanoSeconds() 
			// << " egressbytes " << m_mmu->egress_bytes[ifIndex][downstream_vl]
			// << " node " << GetId() 
			// << " port " << ifIndex
			// << " qIndex " << downstream_vl
			// << std::endl;
			if(m_mmu->pauseCounter[inDev][upstream_vl] == 0)
			{
				Ptr<QbbNetDevice> device = DynamicCast<QbbNetDevice>(m_devices[inDev]);
				device->SendPfc(upstream_vl, 1);
				// std::cout << "send resume " << upstream_vl << std::endl;
			}
		}
	}
	if (1) {
