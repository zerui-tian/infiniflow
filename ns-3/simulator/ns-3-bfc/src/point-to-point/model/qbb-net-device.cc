	...
	for (uint32_t dorr = 0; dorr < 2; dorr++) {
		...
		if (hostDequeueIndex % 2) {
			...
			for (qIndex = 1; qIndex <= fcount; qIndex++) {
				...
				if (!paused[qp->m_vl]&& qp->GetBytesLeft() > 0) {
...
QbbNetDevice::QbbNetDevice()
{
	...
	memset(m_paused, 0, sizeof(m_paused));
}
...
QbbNetDevice::DequeueAndTransmit(void)
{
	...
	if (m_node->GetNodeType() == 0) {
		...
		if (p != 0) {
			...
			TransmitStart(p);
			// std::cout << "transmit " <<  qIndex << std::endl;
			numTxBytes += p->GetSize();
			totalBytesSent += p->GetSize();
			return;
...
QbbNetDevice::Resume(unsigned qIndex)
{
	...
	// std::cout <<"node " << GetNode()->GetId() <<" receive resume qIndex " << qIndex << " m_paused " << m_paused[qIndex] << std::endl;
...
// std::cout << "receive" << std::endl;
	...
	if (ch.l3Prot == 0xFE) { // PFC
		// std::cout <<"node " << GetNode()->GetId() <<" receive pfc frame " << std::endl;
		if (!m_qbbEnabled) return;
		uint16_t qIndex = ch.pfc.qIndex;
		if (ch.pfc.time > 0) {
			m_tracePfc(1,qIndex);
			m_paused[qIndex] = true;
			// std::cout <<"node " << GetNode()->GetId() <<" receive pause qIndex " << qIndex << " m_paused " << m_paused[qIndex] <<std::endl;
		} else {
			m_tracePfc(0,qIndex);
			Resume(qIndex);
		}
...
void QbbNetDevice::SendPfc(uint32_t qIndex, uint32_t type) {
	...
	CustomHeader ch(CustomHeader::L2_Header | CustomHeader::L3_Header | CustomHeader::L4_Header);
	p->PeekHeader(ch);
	m_tracePfc(type+2,qIndex); // 2 indicates PFC PAUSE sent.3 indicates RESUME sent
	// std::cout <<"node " << GetNode()->GetId() <<" send pfc qIndex " << qIndex << std::endl;
	SwitchSend(0, p, ch);
}
