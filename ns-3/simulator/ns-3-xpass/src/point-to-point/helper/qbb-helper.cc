...
void QbbHelper::GetTraceFromPacket(TraceFormat &tr, Ptr<QbbNetDevice> dev, Ptr<const Packet> p, uint32_t qidx, PEvent event, bool hasL2){
	...
	switch (hdr.l3Prot){
		case 0x01:
    tr.cdt.sport = hdr.cdt.sport;
    tr.cdt.dport = hdr.cdt.dport;
    tr.cdt.flags = hdr.cdt.flags;
    tr.cdt.pg = hdr.cdt.pg;
    tr.cdt.m_seq = hdr.cdt.m_seq;
    tr.cdt.m_rtt = hdr.cdt.m_rtt;

    tr.cdt.recv_next = hdr.cdt.recv_next;
    tr.cdt.mtype = hdr.cdt.mtype;

    //cmnh
    tr.cdt.ptype = hdr.cdt.ptype;

    //xph
    tr.cdt.credit_seq = hdr.cdt.credit_seq;
    tr.cdt.credit_sent_time = hdr.cdt.credit_sent_time;
    tr.cdt.sendbuffer = hdr.cdt.sendbuffer;
		default:
			break;
	}
...
void QbbHelper::QpDequeueCallback(FILE *file, Ptr<QbbNetDevice> dev, Ptr<const Packet> p, Ptr<RdmaTxWorkQueue> qp){
	TraceFormat tr;
