      ...
      NS_LOG_TEMPLATE_DEFINE("BEgressQueue")
{
    ...
    for (uint32_t i = 0; i < fCnt; i++)
    {
        ...
        m_queues.push_back(CreateObjectWithAttributes<DropTailQueue<Packet>>(
            "MaxSize",
            QueueSizeValue(QueueSize(BYTES, uint32_t(1000.0 * 1024 * 1024)))));
...
BEgressQueue::DoEnqueue(Ptr<Packet> p, uint32_t qIndex)
{
    ...
    // std::cout << "maxsizeBE " << uint32_t(m_queues[qIndex]->GetMaxSize().GetValue()) << "
    // m_maxBytes " << uint32_t(m_maxBytes) << " double " << m_maxBytes << std::endl;
    // NS_ABORT_MSG("debug");
    ...
    if (m_bytesInQueueTotal + p->GetSize() < m_maxBytes) // infinite queue
    {
        ...
        {
            std::cout << "currentSize " << m_bytesInQueueTotal << " MaxSize "
                      << m_queues[qIndex]->GetMaxSize().GetValue() << std::endl;
