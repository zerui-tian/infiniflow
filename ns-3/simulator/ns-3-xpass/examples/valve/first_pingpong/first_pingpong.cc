...
NS_LOG_COMPONENT_DEFINE("FirstEvaluation");
...
void monitor_link(FILE* throughput_output, NetDeviceContainer *nd) {
	if(Simulator::Now().GetTimeStep() == throughput_mon_start){ //统计开始时清零
		...
		for(uint32_t i = 0; i < nd->GetN(); i++){
			...
			new_item.throughput_Gbps = 
				(double)(output_device->getNumDataBytes() * 8) / throughput_mon_interval;
...
int main(int argc, char *argv[])
{
	...
	std::string confFile = "/home/pnic/valve-ns3/simulator/ns-3.39/examples/valve/first_pingpong/first_pingpong_config.txt";
