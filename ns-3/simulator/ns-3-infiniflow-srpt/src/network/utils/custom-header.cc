...
void CustomHeader::Serialize (Buffer::Iterator start) const{
  ...
  if (headerType & L4_Header){
	  if (l3Prot == 0x6){ // TCP
		  ...
		  i.WriteHtonU16 (udp.vl);
		  i.WriteHtonU32 (udp.fs);
		  udp.ih.Serialize(i);
...
CustomHeader::Deserialize (Buffer::Iterator start)
{
  ...
  if (headerType & L4_Header){
	  if (l3Prot == 0x6){ // TCP
		  ...
		  udp.vl =  i.ReadNtohU16 ();
		  udp.fs = i.ReadNtohU32 ();
		  if (getInt)
...
uint32_t CustomHeader::GetUdpHeaderSize(void){
	return 8 + sizeof(udp.pg) + sizeof(udp.seq) + sizeof(udp.vl)+ sizeof(udp.fs) + IntHeader::GetStaticSize();
}
