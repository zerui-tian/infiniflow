...
// no pkt in highest priority queue, do rr for each qp
	...
	// ---------- SRTF selection ----------
	uint32_t best_bytes_left = 0xffffffff;
	for (qIndex = 0; qIndex < fcount; qIndex++) {
		Ptr<RdmaQueuePair> qp = m_qpGrp->Get(qIndex);
		...
		// if (qp->GetBytesLeft() > 0 && !qp->IsWinBound()) {
			...
			uint32_t pkt_size = 
				(qp->GetBytesLeft() > m_mtu ? m_mtu : qp->GetBytesLeft())
				+ CustomHeader::GetStaticWholeHeaderSize();

			// credit / threshold constraint
			if( credit < pkt_size || (thresholds[qp->m_pg] <= pkt_size + inflight[qp->m_pg]) )
				continue;

			// choose smallest remaining bytes
			if (qp->GetBytesLeft() < best_bytes_left) {
				best_bytes_left = qp->GetBytesLeft();
				res = qIndex;
			}
		}
		else if (qp->IsFinished()) {
			min_finish_id = qIndex < min_finish_id ? qIndex : min_finish_id;
		}
	}
...
QbbNetDevice::DequeueAndTransmit(void)
{
	...
	if (m_node->GetNodeType() == 0) {
		...
		p = m_queue->DequeueSRTP(credit, inflight_counter, threshold);		//this is round-robin
		if (p != 0) {
			m_snifferTrace(p);
