...
uint64_t buffer_size; //BDP = 64250000 Bytes for 25Gbps 5us
...
int main(int argc, char *argv[])
{
	...
	cmd.Parse (argc, argv);
	// std::cout << " cc_windowcheck " << windowCheck << std::endl;
	...
	// uint32_t leaf_spine_bdp =  25 * 5000 * 2 / 8; // 100Gbps, 500ns delay, 1 BDP(bytes)
	// uint32_t per_port_buffer = 4 * SwitchMmu::qCnt * (leaf_spine_bdp); // per port buffer size **这个修改为自己设置的buffer
	uint32_t per_port_buffer = (16 * 100 * 1000 * 2 / 8);
	buffer_size = SwitchMmu::pCnt * per_port_buffer; // total buffer size for each switch
	for (uint32_t i = 0; i < node_num; i++) {
	...
	// std::vector<uint32_t> target_fids = {10000};
