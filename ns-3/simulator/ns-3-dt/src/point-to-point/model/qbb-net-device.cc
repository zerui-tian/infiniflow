...
int RdmaEgressQueue::GetNextQindex(uint32_t credit, uint32_t inflight[], uint32_t threshold) {
	bool found = false;
	uint32_t qIndex;
	...
	if (m_fcpQ->GetNPackets() > 0) {
		return -2;
			...
			(threshold - inflight[1]) >= m_ackQ->Peek()->GetSize()
		){
		return -1;
	}
	...
	for (qIndex = 1; qIndex <= fcount; qIndex++) {
		...
		// if (qp->GetBytesLeft() > 0 && !qp->IsWinBound()) {
			...
			if( credit < pkt_size || (threshold < pkt_size + inflight[qp->m_pg]) ) // no enough credits
				continue;
			res = idx;
			break;
		}
...
QbbNetDevice::DequeueAndTransmit(void)
{
	...
	if (m_node->GetNodeType() == 0) {
		...
		uint32_t threshold = credit * alpha;
		...
		qIndex = m_rdmaEQ->GetNextQindex(credit, inflight_counters, threshold);
...
// std::cout << "receive" << std::endl;
	...
	if(pppProto == 0x0086) {
		if (m_node->GetNodeType() > 0) {
			...
			// 	}
			// 	else if(qlen <= qlen_min){
			// 		new_threshold = perpg_inflight_threshold[qIndex] + ai_step;
