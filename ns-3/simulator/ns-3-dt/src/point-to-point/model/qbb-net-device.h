...
public:
  ...
  Ptr<DropTailQueue<Packet>> m_fcpQ;
	...
	// int GetNextQindex(uint32_t credit, uint32_t inflight[], uint32_t thresholds[]);
  int GetNextQindex(uint32_t credit, uint32_t inflight[], uint32_t threshold);
	int GetLastQueue();
...
public:
  ...
  uint64_t getNumTxBytesByFlow(uint32_t flow_id) {
    ...
    //cbfc
  uint32_t perpt_fctbs; //per priority queue每个队列发送的
  uint32_t perpt_fccl;  //per priority queue每个队列接收的下游fccl值

  uint32_t perpq_accumu_used_credit[qCnt]; 
	uint32_t perpq_accumu_recycled_credit[qCnt]; 
	uint32_t perpq_accumu_drained_bytes[qCnt]; 

  // uint32_t perpg_inflight_threshold[qCnt];
  uint32_t inflight_threshold;
  uint32_t maxThreshold;

  bool need_set_tap[qCnt];
  bool need_ret_tap[qCnt];
  bool rate_adjusting[qCnt];

  void SetInflightThreshold(uint32_t qIndex, uint32_t threshold);
