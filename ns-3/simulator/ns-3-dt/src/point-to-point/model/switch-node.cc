...
SwitchNode::SendToDev(Ptr<Packet> p, CustomHeader& ch)
{
    ...
    if (idx >= 0)
    {
        ...
        {
            qIndex = ch.udp.pg; // For TCP/IP if the stack did not attach
        ...
        { // Admission control
            ...
            if(tap){
                ...
                p->AddHeader(h);
                p->AddHeader(ppp);
                m_mmu->need_ret_tap[inDev][qIndex] = true;
            }

            m_bytes[inDev][idx][qIndex] += p->GetSize();
            m_devices[idx]->SwitchSend(qIndex, p, ch);
            DynamicCast<QbbNetDevice>(m_devices[idx])->totalBytesRcvd +=
                p->GetSize(); // Attention: this is the egress port's total received packets. Not
                            // the ingress port.
        }
    }
    else
...
SwitchNode::SwitchReceiveFcp(Ptr<Packet> p,uint32_t port) {
	...
	// 		// 		 <<  std::endl;
    //     }
    //     else if(qlen <= m_mmu->qlen_min){
    //         new_threshold = m_mmu->inflight_threshold[port][qIndex] + m_mmu->ai_step;
...
SwitchNode::SwitchNotifyDequeue(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p)
{
    ...
    Ptr<Packet> fcp = m_mmu->ConstructFcp(inDev, qIndex);
    Ptr<QbbNetDevice> dev = DynamicCast<QbbNetDevice>(m_devices[inDev]);
    dev->SendFcp(fcp);
    // std::cout << "Time " << Simulator::Now().GetNanoSeconds() 
	// 		<< " ingress_bytes " << m_mmu->ingress_bytes[inDev][qIndex]
    //         << " node " << GetId()
	// 		<< " port " << ifIndex
	// 		<< " qIndex " << qIndex
	// 		<< std::endl;
    ...
    if(m_mmu->need_set_tap[ifIndex][qIndex]){
        PppHeader ppp;
        Ipv4Header h;
        p->RemoveHeader(ppp);
        p->RemoveHeader(h);
        ...
        p->AddHeader(h);
        p->AddHeader(ppp);
        m_mmu->need_set_tap[ifIndex][qIndex] = false;
    }
    else{
        PppHeader ppp;
        Ipv4Header h;
