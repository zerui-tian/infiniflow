...
public:
	...
	static const uint32_t qCnt = 8;	// Number of queues/priorities used 不是const，变为了可配置的
	...
	uint32_t fctbs[pCnt]; //vl已发送字节数
	uint32_t abr[pCnt]; //vl已接收字节数
	uint32_t fccl[pCnt]; //上游端口接收的下游端口的remaining buffer + abr
	uint32_t buffer_size[pCnt]; //vl可用的buffer大小
	uint32_t used_bytes[pCnt]; //当前port已使用的字节数
