...
namespace ns3 {
	...
	Ptr<Packet>
		BEgressQueue::DoDequeueRR(uint32_t credit, uint32_t inflight[], uint32_t threshold) //this is for switch only
	{
		...
		else if (m_queues[1]->GetNPackets() > 0) //1 is the ACK priority
		{
			...
			if (credit >= pkt_len && threshold >= (pkt_len + inflight[1])){
				found = true;
				qIndex = 1;
			}
		}
		...
		if (!found) {
			for (qIndex = 1; qIndex <= qCnt; qIndex++)
			{
				...
				if(m_queues[cur_q_index]->GetNPackets() > 0){
					...
					if (credit >= pkt_len && threshold >= (pkt_len + inflight[cur_q_index]))  //round robin
					{
						found = true;
						break;
					}
	...
	Ptr<Packet>
		BEgressQueue::DequeueRR(uint32_t credit, uint32_t inflight[], uint32_t threshold)
	{
		NS_LOG_FUNCTION(this);
		Ptr<Packet> packet = DoDequeueRR(credit, inflight, threshold);
//		if (packet != 0)
		// {
