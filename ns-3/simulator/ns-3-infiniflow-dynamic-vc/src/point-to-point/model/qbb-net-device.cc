...
Ptr<Packet> RdmaEgressQueue::DequeueQindex(int qIndex) {
	...
	if (qIndex == -1) { // ack
		...
		m_rrlast = qIndex;
		m_qlast = qIndex;
		m_traceRdmaDequeue(p, m_qpGrp->Get(qIndex)->m_vl);
	}
	else {
		p = 0;
	}
...
int RdmaEgressQueue::GetNextQindex(uint32_t credit, uint32_t inflight[], uint32_t thresholds[]) {
	...
	for (qIndex = 1; qIndex <= fcount; qIndex++) {
		...
		// if (qp->GetBytesLeft() > 0 && !qp->IsWinBound()) {
			...
			// no enough credit
			if( credit < pkt_size || (thresholds[qp->m_vl] <= pkt_size + inflight[qp->m_vl]) ) 
			{
				continue;
			}
			res = idx;
...
QbbNetDevice::DequeueAndTransmit(void)
{
	...
	if (m_node->GetNodeType() == 0) {
		...
		//注意qIndex和优先级的区别
		//qIndex是端系统对自身队列和QP的编号，ack队列是-1，fcp队列是-2，其他QP从0开始编号
		//优先级才是交换机处所属队列的编号，fcp包是0，ack是1，其他包从2开始
		...
		if (qIndex != -1024) {
			...
			if(qIndex == -2) { // fcp
				...
				perpq_accumu_used_credit[lastQp->m_vl] += p->GetSize();
				if(need_set_tap[lastQp->m_vl]){
					PppHeader ppp;
					Ipv4Header h;
					p->RemoveHeader(ppp);
					p->RemoveHeader(h);
					h.SetTap();
					// std::cout << "set tap for ack packet at node " << m_node->GetId() << " at port " << GetIfIndex() << "qIndex " << lastQp->m_vl << std::endl;
					p->AddHeader(h);
					p->AddHeader(ppp);
					need_set_tap[lastQp->m_vl] = false;
				}
				else {
					PppHeader ppp;
					Ipv4Header h;
					...
					p->RemoveHeader(h);
					h.ResetTap();
					// std::cout << "reset tap for ack packet at node " << m_node->GetId() << " at port " << GetIfIndex() << "qIndex " << lastQp->m_vl << std::endl;
					p->AddHeader(h);
					p->AddHeader(ppp);
				}
				...
				std::cout << "Time " << Simulator::Now().GetNanoSeconds() << " fccl " << perpt_fccl <<
				 " fctbs " << perpt_fctbs <<
				 " perpq_accumu_used_credit " << perpq_accumu_used_credit[lastQp->m_vl] << 
				 " perpq_accumu_recycled_credit " << perpq_accumu_recycled_credit[lastQp->m_vl] <<
                 " inflight " << perpq_accumu_used_credit[lastQp->m_vl] - perpq_accumu_recycled_credit[lastQp->m_vl]
				 << " node " << m_node->GetId() 
				 << " port " << GetIfIndex() << " qIndex " << lastQp->m_vl <<  std::endl;
		...
		if(sw->m_mmu->fccl[m_ifIndex] >= sw->m_mmu->fctbs[m_ifIndex]){
			...
			std::cout << "error! negative credit!" << sw->m_mmu->fccl[m_ifIndex] << " " << sw->m_mmu->fctbs[m_ifIndex] <<std::endl;
			credit = 0;
		}
		...
		for(int i = 0; i < qCnt; i++){
			// std::cout << m_ifIndex << " i " << i << " accumu_used_credit " << sw->m_mmu->accumu_used_credit[m_ifIndex][i]
			// << " accumu_recycled_credit " << sw->m_mmu->accumu_recycled_credit[m_ifIndex][i] << std::endl;
			if(sw->m_mmu->accumu_used_credit[m_ifIndex][i] >= sw->m_mmu->accumu_recycled_credit[m_ifIndex][i]){
				...
				// std::cout << "error! negative inflight_counter!in node"<< GetNode()->GetId() << " port " << m_ifIndex << " qIndex " << i <<std::endl;
				// std::cout << " accumu_used_credit " << sw->m_mmu->accumu_used_credit[m_ifIndex][i]
				// << " accumu_recycled_credit " << sw->m_mmu->accumu_recycled_credit[m_ifIndex][i] << std::endl;
			}
		...
		if (p != 0) {
			...
			if(qIndex != 0) {// is not fcp
				sw->SwitchNotifyDequeue(m_ifIndex, qIndex, p);
			}
...
// std::cout << "receive" << std::endl;
	...
	if(pppProto == 0x0086) {
		if (m_node->GetNodeType() > 0) {
			//交换机端收到fcp包该去更新该优先级队列的Fccl值
			DynamicCast<SwitchNode>(m_node)->SwitchReceiveFcp(packet, m_ifIndex);
		}
		else {
			/*主机端收到fcp包去更新其相应的priority的fccl值并去判断是否可以发下一个包 10/21 by sgh*/
			PppHeader ph;
			packet->RemoveHeader(ph);
			FcpHeader fh;
			packet->RemoveHeader(fh);
			// std::cout << "[FIND] map address = " << &flow_qIndex_map << std::endl;
			...
			// std::cout << "receive fcp Node " << m_node->GetId() << " port " << GetIfIndex() << " qIndex " 
			// << qIndex << "perpq_accumu_recycled_credit " << perpq_accumu_recycled_credit[qIndex] << std::endl;
			// std::cout << "f " << fccr << std::endl ;
			if(tap){
				rate_adjusting[qIndex] = false;
			}
			if(!rate_adjusting[qIndex]) {
		...
		if (m_node->GetNodeType() > 0) { // switch
			...
			//设置传递的字节数;
			fcph.SetFccl(0xffffffff);
			fcph.SetQlen(0);
			fcph.SetFccr(perpq_accumu_drained_bytes[vl]);
			Ptr<Packet> fcp = Create<Packet> (0);
			// std::cout << "NIC send fcp Node " << m_node->GetId() << " port " << GetIfIndex() << " qIndex " 
			// << vl << " perpq_accumu_drained_bytes " << perpq_accumu_drained_bytes[vl] << std::endl;
			if(tap){
				fcph.SetTap();
			}
			else{
