...
uint32_t RdmaHw::GetNicIdxOfQp(Ptr<RdmaQueuePair> qp) {
	...
	if (v.size() > 0) {
		// std::cout << "nic idx vec size: " << v.size() << std::endl;
...
void RdmaHw::AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish, Time stopTime) {
	...
	if(i == QbbNetDevice::qCnt){
		int range = QbbNetDevice::qCnt - 2;
		uint32_t vl = 2 + (rand() % range);
...
int RdmaHw::ReceiveUdp(Ptr<Packet> p, CustomHeader &ch) {
	...
	// std::cout << "host reiceive udp packet size: " << p->GetSize() << std::endl;
	...
	if (x == 1 || x == 2) { //generate ACK or NACK
		...
		m_nic[nic_idx].dev->RdmaEnqueueAckQ(newp);
		
		m_nic[nic_idx].dev->TriggerTransmit();
		//为ack流也创建flow_qIndex_map
		if(!m_nic[nic_idx].dev->vl_states[1]) {
			m_nic[nic_idx].dev->vl_states[1] = true;
			m_nic[nic_idx].dev->flow_qIndex_map[1] = 1;
			// std::cout << "[INSERT] map address = " << &m_nic[nic_idx].dev->flow_qIndex_map << std::endl;
		}
	}
	return 0;
...
Ptr<Packet> RdmaHw::GetNxtPacket(Ptr<RdmaQueuePair> qp) {
	...
	qp->m_ipid++;
	// return
	qp->numTxBytes = qp->numTxBytes + p->GetSize();
	return p;
}
