...
void SwitchMmu::RemoveFromIngressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize) {
	...
	//这里是对应的remaining buffer size改变了
	#if OUTPUT_1
...
//发送credit包
Ptr<Packet> SwitchMmu::ConstructFcp(uint32_t inDev, uint32_t upstream_vl) {
	FcpHeader fcph = FcpHeader();
	fcph.SetVl(upstream_vl);
	// std::cout << "Construct FCP at node " << node_id << " port " << inDev << " set vl " << upstream_vl << std::endl;
	//设置传递的字节数
	uint32_t fccl_ =  abr[inDev] + (buffer_size[inDev] - used_bytes[inDev]);
	fcph.SetFccl(fccl_);
	fcph.SetQlen(ingress_bytes[inDev][upstream_vl]);
	fcph.SetFccr(accumu_drained_bytes[inDev][upstream_vl]);
	if(need_ret_tap[inDev][upstream_vl]){
		fcph.SetTap();
		need_ret_tap[inDev][upstream_vl] = false;
	}
	else{
		fcph.ResetTap();
	}
...
void SwitchMmu::SetBufferPool(uint64_t b) {
	...
	for(int p = 0; p < pCnt; p++){
		...
		fccl[p] = buffer_size[p]; //这里本应该是每个port对侧的fccl，但是由于我们仿真中所有链路都设置为相同参数，因此，直接使用这个值即可
	}
}
