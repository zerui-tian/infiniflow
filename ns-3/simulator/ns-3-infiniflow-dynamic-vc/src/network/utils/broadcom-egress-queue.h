...
namespace ns3 {
	...
	public:
		...
		Ptr<Packet> DequeueRR(uint32_t credit, uint32_t inflight[], uint32_t thresholds[]);
	...
	private:
		...
		Ptr<Packet> DoDequeueRR(uint32_t credit, uint32_t inflight[], uint32_t thresholds[]);
		//for compatibility
