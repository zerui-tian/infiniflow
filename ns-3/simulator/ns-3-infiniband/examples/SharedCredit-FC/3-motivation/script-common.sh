...
############################################
# BUFFER_SIZE_LIST=(20 50 100 200 500 1000)     # 外层：buffer
...
for buffer_mul in "${BUFFER_SIZE_LIST[@]}"; do
    ...
    for vc_cnt in "${VC_CNT_LIST[@]}"; do
        ...
        ############################################
        FCT_FILE="$OUTPUT/3-motivation/buf${buffer_mul}_vc${vc_cnt}_cbfc.txt"
        STDOUT_FILE="$output_dir/buf${buffer_mul}_vc${vc_cnt}_cbfc.out"
        ...
        # Run experiment
        ############################################
        ./waf --run "3-motivation --conf=$config_file" \
            > "$STDOUT_FILE" || {
        ...
        # gdb --args build/examples/SharedCredit-FC/ns3.39-3-motivation-optimized --conf=$config_file
    done
done
