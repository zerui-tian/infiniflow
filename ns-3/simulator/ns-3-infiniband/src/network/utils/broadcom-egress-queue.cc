...
namespace ns3 {
		...
		Queue(), NS_LOG_TEMPLATE_DEFINE("BEgressQueue")
	{
		...
		{
			// std::cout << "error in be " << i << std::endl;
			m_bytesInQueue[i] = 0;
			// std::cout << "inqueue = 0 " << i << std::endl;
	...
	Ptr<Packet>
		BEgressQueue::DoDequeueRR(uint32_t credits[]) //this is for switch only
	{
		...
		if (!found) {
			for (qIndex = 1; qIndex <= qCnt; qIndex++)
			{
				...
				if(m_queues[cur_q_index]->GetNPackets() > 0){
					...
					if (credits[cur_q_index] >= pkt_len)  //round robin
					{
						// std::cout << " sw : credit " << credits[cur_q_index] << " >= " << pkt_len << std::endl;
						found = true;
						break;
					}
				}
	...
	Ptr<Packet>
		BEgressQueue::DequeueRR(uint32_t credits[])
	{
		NS_LOG_FUNCTION(this);
		Ptr<Packet> packet = DoDequeueRR(credits);
//		if (packet != 0)
		// {
