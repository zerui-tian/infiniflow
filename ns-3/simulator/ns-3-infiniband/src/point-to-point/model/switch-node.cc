...
SwitchNode::SwitchReceiveFcp(Ptr<Packet> p,uint32_t port) {
    ...
    m_mmu->fccl[port][qIndex] = fccl;
    #if OUTPUT_1
        std::cout << "Time " << Simulator::Now().GetNanoSeconds() << " fccl " << m_mmu->fccl[port][qIndex] <<
                    " fctbs " << m_mmu->fctbs[port][qIndex] << 
                    " node " << GetId() << " port " << port << " qIndex " << qIndex << std::endl;
    #endif
