...
int RdmaEgressQueue::GetNextQindex(uint32_t credits[]) {
	bool found = false;
	uint32_t qIndex;
	if (m_fcpQ->GetNPackets() > 0){
		return -2;
	}
	if (m_ackQ->GetNPackets() > 0 && credits[1] >= m_ackQ->Peek()->GetSize() ){
		return -1;
	}
	...
	for (qIndex = 1; qIndex <= fcount; qIndex++) {
		...
		// if (qp->GetBytesLeft() > 0 && !qp->IsWinBound()) {
			...
			if( credits[qp->m_pg] < pkt_size ){ // no enough credits
				continue;
			}
			// std::cout<< "hw : credit " << credits[qp->m_pg] << " >= " << pkt_size << std::endl;
			res = idx;
			break;
		}
...
QbbNetDevice::QbbNetDevice()
{
	...
	memset(perpq_fctbs, 0, sizeof(perpq_fctbs));
}
...
QbbNetDevice::DequeueAndTransmit(void)
{
	...
	if (m_node->GetNodeType() == 0) {
		uint32_t credits[qCnt];
		for(int i = 1; i < qCnt; i++) {
			if(perpq_fccl[i] < perpq_fctbs[i]) {
				std::cout << "spill" << std::endl;
			}
			credits[i] = perpq_fccl[i] - perpq_fctbs[i];
		}

		//注意qIndex和优先级的区别
		//qIndex是端系统对自身队列和QP的编号，ack队列是-1，fcp队列是-2，其他QP从0开始编号
		//优先级才是交换机处所属队列的编号，fcp包是0，ack是1，其他包从2开始
		qIndex = m_rdmaEQ->GetNextQindex(credits);
		...
		if (qIndex != -1024) {
			...
			if(qIndex == -2) { // fcp
				...
				TransmitStart(p);
				totalBytesSent += p->GetSize();
				perpq_fctbs[1] += p->GetSize();
				#if OUTPUT_1
					std::cout << "Time " << Simulator::Now().GetNanoSeconds() << " fccl " << perpq_fccl[1] <<
					" fctbs " << perpq_fctbs[1] << " node " << m_node->GetId() 
				...
				TransmitStart(p);
				totalBytesSent += p->GetSize();
				// std::cout << "hw send packet pg : " << lastQp->m_pg <<std::endl;
				perpq_fctbs[lastQp->m_pg] += p->GetSize();

				// update for the next avail time
				m_rdmaPktSent(lastQp, p, m_tInterframeGap);
				#if OUTPUT_1
					std::cout << "Time " << Simulator::Now().GetNanoSeconds() << " fccl " << perpq_fccl[lastQp->m_pg] <<
					" fctbs " << perpq_fctbs[lastQp->m_pg] << " node " << m_node->GetId() 
		...
		for (uint32_t i = 1; i < qCnt; i++) {
			if(sw->m_mmu->fccl[m_ifIndex][i] < sw->m_mmu->fctbs[m_ifIndex][i]){
				std::cout << "spill" << std::endl;
			}	
			credits[i] = sw->m_mmu->fccl[m_ifIndex][i] - sw->m_mmu->fctbs[m_ifIndex][i];
		}
		p = m_queue->DequeueRR(credits);		//this is round-robin
		if (p != 0) {
			m_snifferTrace(p);
			...
			p->PeekHeader(ch);
			// std::cout << "sw send pg " << ch.udp.pg <<std::endl;
 			uint32_t qIndex = m_queue->GetLastQueue();
			if(qIndex != 0) {// is not fcp			
				m_node->SwitchNotifyDequeue(m_ifIndex, qIndex, p);
				p->RemovePacketTag(t);
				p->RemovePacketTag(ts);
				
				// if (GetNode()->GetId() == target_swid && ch.l3Prot == 0x11 && ch.udp.dport == target_fid){
				// 	std::cout << "queueIndelay " << Simulator::Now().GetNanoSeconds() - ts.GetTimestamp().GetNanoSeconds() << std::endl;
				// }
			}
...
QbbNetDevice::Receive(Ptr<Packet> packet)
{
	...
	if(pppProto == 0x0086) {
		if (m_node->GetNodeType() > 0) {
			...
			perpq_fccl[qIndex] = fccl;
			DequeueAndTransmit(); 
			#if OUTPUT_1			
				std::cout << "Time " << Simulator::Now().GetNanoSeconds() << " fccl " << perpq_fccl[qIndex] <<
					" fctbs " << perpq_fctbs[qIndex] << " node " << m_node->GetId() 
					<< " port " << GetIfIndex() << " qIndex " << qIndex << std::endl;
			#endif
		}
	}
	else {
		...
		if (m_node->GetNodeType() > 0) { // switch
			...
			packet->AddPacketTag(TimestampTag(Simulator::Now()));
			// std::cout << "sw receive pg " << ch.udp.pg << std::endl;
