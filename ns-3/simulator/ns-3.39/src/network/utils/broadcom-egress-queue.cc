...
namespace ns3 {
	...
	Ptr<Packet>
		BEgressQueue::DoDequeueRR(uint32_t credit, uint32_t inflight[], uint32_t thresholds[]) //this is for switch only
	{
		...
		if (m_queues[0]->GetNPackets() > 0) //0 is the BCP priority
		{
			found = true;
			qIndex = 0;
		}
		else if (m_queues[1]->GetNPackets() > 0) //1 is the ACK priority
		{
			uint32_t pkt_len = m_queues[1]->Peek()->GetSize();
			if (credit >= pkt_len && thresholds[1] >= (pkt_len + inflight[1])){
				found = true;
				qIndex = 1;
			}
		}
		...
		if (!found) {
			...
			{
				uint32_t cur_q_index = (qIndex + m_rrlast) % qCnt;
				if(m_queues[cur_q_index]->GetNPackets() > 0){

					uint32_t pkt_len = m_queues[cur_q_index]->Peek()->GetSize();
					if (credit >= pkt_len && thresholds[cur_q_index] > (pkt_len + inflight[cur_q_index]))  //round robin
					{
						found = true;
						break;
					}
		...
		if (found)
		{
			// std::cout << "dequeue from qIndex " << qIndex << std::endl;
	...
	Ptr<Packet>
		BEgressQueue::DequeueRR(uint32_t credit, uint32_t inflight[], uint32_t thresholds[])
	{
		NS_LOG_FUNCTION(this);
		Ptr<Packet> packet = DoDequeueRR(credit, inflight, thresholds);
//		if (packet != 0)
		// {
